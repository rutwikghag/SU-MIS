...
```{r}
library(tidyverse)
file <- "https://intro-datascience.s3.us-east-2.amazonaws.com/companies1.csv" # As taught in class
help("read_csv")
dfComps <- read_csv(file)
dfComps
```
...
head(dfComps)
tail(dfComps)
#we see a data frame of organizations in the US along with their details like URLs, last funding, etc
...
```{r}
dfComps$noURL <- ifelse(is.na(dfComps$homepage_url), TRUE, FALSE) 
view(dfComps)
...
```{r}
urlComps <- dfComps[dfComps$noURL == FALSE,]
```
...
```{r}
newdfComps <- dfComps[dfComps$noURL == TRUE,]
mean(newdfComps$funding_rounds)
...
urlComps$company <- str_replace_all(urlComps$permalink, "/organization/", "")
head(urlComps)
```
...
```{r}
as.numeric(urlComps$funding_total_usd)[1:50]

```
# The output is NA as we have spaces there
...
```{r}
library(stringi)
convertCharToNum <- function(char_string) {     # Initialize a function to convert numbers to numbers
  step1 <- stri_replace_all_charclass(char_string,"\\p{WHITE_SPACE}", "") # Remover whitespace
  step2 <- as.numeric(step1)  # Converts them to numeric and stores the new series into step2
  return(step2)
}
```
...
convertCharToNumNew <- function(char_string) {
  step1 <- str_replace_all(char_string, pattern = "\\p{WHITE_SPACE}", repl="")
...
```{r}
urlComps$result <- convertCharToNumNew(urlComps$funding_total_usd)
```
...
```{r}
mean(urlComps$result, na.rm = TRUE)
```
